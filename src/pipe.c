...
int instruReadMemCount = 50;
int dCacheReadMemCount = 50;
int dCacheWriteMemCount_SB = 50;
int dCacheWriteMemCount_SH = 50;
int dCacheWriteMemCount_SW = 50;
//#define DEBUG
...
void pipe_stage_mem()
{
    ...
    if (op->is_mem){
        uint32_t result = data_cache_read_32(op->mem_addr & ~3);
        if(result == 0x0381CD55)
            return;
        val = result;
    }
    ...
    switch (op->opcode) {
        ...
        case OP_LBU:
            ...
            break;
        //由于已经执行了前面过的读cache，下面去写的时候好像不会不命中
        case OP_SB:{
            switch (op->mem_addr & 3) {
            ...
            int result1 = data_cache_write_32(op->mem_addr & ~3, val);//result应该不会是0
            if(result1 == 0){
                //printf("yingyingyingying1\n");
                return; 
            }
...
void pipe_stage_fetch()
{
    ...
    //延时操作,instruReadMemCount为全局变量，初值50
    if (instruReadMemCount > 0 && instruReadMemCount < 50)
    { //正在延时

        instruReadMemCount--;
        // printf("delaying instruReadMemCount = %d\n", instruReadMemCount);
        // printf("now pipe.PC = %08x\n", pipe.PC);

        return;
    }
    if (instruReadMemCount == 0)
    { //延时结束，直接替换
        // printf("delaying over! instruReadMemCount = %d\n", instruReadMemCount);
        Pipe_Op *op = malloc(sizeof(Pipe_Op));
        memset(op, 0, sizeof(Pipe_Op));
        op->reg_src1 = op->reg_src2 = op->reg_dst = -1;

        op->instruction = instru_cache_miss_todo(pipe.PC); //指令cache缺失替换操作
        op->pc = pipe.PC;
        pipe.decode_op = op;
    ...
    uint32_t theMem = is_instru_cache_hit(pipe.PC); //命中时为指令，没命中为全0
    if (theMem == 0x0381CD55)
    { //没命中
        instruReadMemCount--;
        return;
    }
